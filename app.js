...
function statusBadge(status) {
  ...
  return `<span class="badge ${map[status] || 'badge-petrol'}">${status}</span>`;
}

function tipoColor(tipo) {
  const c = { 'Preventiva':'#1a7a8a','Corretiva':'#dc2626','Revisão':'#2563eb','Troca de Óleo':'#16a34a','Pneus':'#d97706','Freios':'#9333ea','Elétrica':'#f0a500','Outro':'#64748b' };
...
function renderDashboard() {
  const hoje = new Date();
  document.getElementById('kpi-total').textContent    = manutencoes.length;
  document.getElementById('kpi-custo').textContent    = formatCurrency(manutencoes.reduce((a, m) => a + Number(m.custo || 0), 0));
  document.getElementById('kpi-veiculos').textContent = veiculos.length;
  document.getElementById('kpi-mes').textContent      = manutencoes.filter(m => {
  ...
  const tipoMap = {};
  manutencoes.forEach(m => { tipoMap[m.tipo] = (tipoMap[m.tipo] || 0) + Number(m.custo || 0); });
  ...
  chartEl.innerHTML = sorted.length
    ? sorted.map(([tipo, val]) => `
        <div class="chart-bar-row">
          ...
          <div class="chart-bar-track"><div class="chart-bar-fill" style="width:${(val/max*100).toFixed(1)}%;background:linear-gradient(90deg,${tipoColor(tipo)}99,${tipoColor(tipo)})"></div></div>
          <span class="chart-bar-value">${formatCurrency(val)}</span>
        </div>`).join('')
    : '<p class="chart-empty">Nenhum dado disponível</p>';

  const ultimas = [...manutencoes].sort((a, b) => (b.data || '').localeCompare(a.data || '')).slice(0, 6);
  const ulEl = document.getElementById('ultimasManutencoes');
  ulEl.innerHTML = ultimas.length
    ? ultimas.map(m => {
    ...
    : '<p class="chart-empty">Nenhum registro ainda</p>';
}
...
function renderTable() {
  ...
  const list = manutencoes.filter(m => {
    const v = getVeiculo(m.veiculoId);
    return (!filterTipo || m.tipo === filterTipo) &&
      (!filterSearch ||
  ...
  if (!list.length) { tbody.innerHTML = ''; emptyEl.style.display = 'flex'; return; }
    ...
    return `<tr>
      ...
      <td>
        <div class="td-actions">
          <button type="button" class="btn-icon btn-edit" onclick="editManutencao(${origIdx})" title="Editar">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </button>
          <button type="button" class="btn-icon btn-delete" onclick="deleteManutencao(${origIdx})" title="Excluir">
...
function renderVeiculos() {
  ...
  if (!veiculos.length) { grid.innerHTML = ''; emptyEl.style.display = 'flex'; return; }
  emptyEl.style.display = 'none';
  grid.innerHTML = veiculos.map((v, i) => {
    const count = manutencoes.filter(m => m.veiculoId === v.id).length;
    const custo = manutencoes.filter(m => m.veiculoId === v.id).reduce((a, m) => a + Number(m.custo || 0), 0);
    return `<div class="veiculo-card">
      <div class="veiculo-card-header">
        ...
        <button type="button" class="btn-icon btn-delete" onclick="deleteVeiculo(${i})" title="Remover">
      ...
      <div class="veiculo-meta">
        <div class="veiculo-meta-item"><span class="veiculo-meta-label">Ano</span><span class="veiculo-meta-value">${v.ano || '–'}</span></div>
        <div class="veiculo-meta-item"><span class="veiculo-meta-label">Cor</span><span class="veiculo-meta-value">${v.cor || '–'}</span></div>
        <div class="veiculo-meta-item"><span class="veiculo-meta-label">Setor</span><span class="veiculo-meta-value">${v.setor || '–'}</span></div>
        <div class="veiculo-meta-item"><span class="veiculo-meta-label">Responsável</span><span class="veiculo-meta-value">${v.resp || '–'}</span></div>
      </div>
      ...
      <div class="veiculo-card-actions">
        <button type="button" class="btn-secondary" style="font-size:12px;padding:6px 12px" onclick="newManutForVeiculo('${v.id}')">
...
function populateVeiculoSelect() {
  document.getElementById('formVeiculo').innerHTML =
...
function newManutForVeiculo(veiculoId) {
  ...
  setTimeout(() => { openModal(); document.getElementById('formVeiculo').value = veiculoId; }, 80);
}
...
function saveVeiculo() {
  ...
  if (!modelo || !placa) { showToast('⚠️ Modelo e Placa são obrigatórios.'); return; }
  veiculos.push({
    id: uid(), modelo,
...
function deleteVeiculo(idx) {
  ...
  const count = manutencoes.filter(m => m.veiculoId === v.id).length;
  if (!confirm(count > 0
    ? `Este veículo possui ${count} registro(s). Deseja excluir mesmo assim?`
    : 'Deseja excluir este veículo?')) return;
  manutencoes = manutencoes.filter(m => m.veiculoId !== v.id);
  veiculos.splice(idx, 1);
  save(); renderVeiculos(); renderDashboard();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    const logado = JSON.parse(sessionStorage.getItem('fc_logado') || '{}');
    if (logado.nome) {
      const primeiro = logado.nome.split(' ')[0];
      document.getElementById('userName').textContent   = primeiro;
      document.getElementById('userAvatar').textContent = primeiro[0].toUpperCase();
    }
  } catch (e) {}
  ...
  // Fechar modais no overlay (clique fora) e Escape
  document.getElementById('modalOverlay').addEventListener('click', function(e) {
    if (e.target === this) closeModal();
  });
  document.getElementById('veiculoModalOverlay').addEventListener('click', function(e) {
    if (e.target === this) closeVeiculoModal();
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeModal(); closeVeiculoModal(); }
  });
